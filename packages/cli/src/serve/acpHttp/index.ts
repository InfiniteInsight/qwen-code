...
export interface MountAcpHttpOptions {
  ...
  /** File system factory for _qwen/file/* methods. Optional; methods 503 without it. */
  fsFactory?: WorkspaceFileSystemFactory;
  /** Device flow registry for _qwen/workspace/auth/* methods. Optional; status returns empty without it. */
...
export function mountAcpHttp(
  ...
  app.post(path, async (req: Request, res: Response) => {
    ...
    if (!conn) {
      // RFD: unknown connection → 404 Not Found.
      res
        .status(404)
        .json(
          rpcError(
