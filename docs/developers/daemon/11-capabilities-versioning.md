...
Auth：`auth_device_flow`、**`require_auth`**（条件）、**`allow_origin`**（条件）。
...
- Toggle 输入：`runQwenServe` 构造 `{ requireAuth, mcpPoolActive, allowOriginActive, promptDeadlineMs, writerIdleTimeoutMs }` 透传到 envelope。
